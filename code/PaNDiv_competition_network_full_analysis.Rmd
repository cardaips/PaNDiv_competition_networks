...
alpha_june_control<-alpha_june_control[c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt"),]
alpha_june_control<-alpha_june_control[,c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                               "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt")]

alpha_june_nitrogen<-alpha_june_nitrogen[c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                               "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt"),]
alpha_june_nitrogen<-alpha_june_nitrogen[,c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                                "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt")]

alpha_june_fungicide<-alpha_june_fungicide[c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                               "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt"),]
alpha_june_fungicide<-alpha_june_fungicide[,c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                                "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt")]

alpha_june_combined<-alpha_june_combined[c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                               "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt"),]
alpha_june_combined<-alpha_june_combined[,c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                                "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt")]

alpha_august_control<-alpha_august_control[c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                               "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt"),]
alpha_august_control<-alpha_august_control[,c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                                "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt")]

alpha_august_nitrogen<-alpha_august_nitrogen[c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                                 "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt"),]
alpha_august_nitrogen<-alpha_august_nitrogen[,c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                                  "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt")]

alpha_august_fungicide<-alpha_august_fungicide[c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                                   "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt"),]
alpha_august_fungicide<-alpha_august_fungicide[,c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                                    "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt")]

alpha_august_combined<-alpha_august_combined[c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                                 "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt"),]
alpha_august_combined<-alpha_august_combined[,c("Am","Sp","Cj","Dc","Pm","Pg","Ao","Be","Fr","Hp",
                                  "To","Cb","Ga","Ra","Dg","Hl","Lp","Pt")]
...
```{r, figure 1 interaction matrices, fig.width = 8, fig.height = 14, fig.align ='center', echo = FALSE}

ggarrange(contju, niju, funju, combju, contau,niau, funau, combau, common.legend = T, ncol = 2, nrow =4)
...
This code presents the average effect across June and August 2020, but with a few modifications (as indicated in the code below), you can look at the results on the two sampling periods separately and obtain the Supplementary figures 2 and 3. 
...
datapred<-slapred(m2,comp) # takes two-three minutes to run, personal function defined in "function_competition_network.R"
#average across seasons for more clarity, remove to look at the two sampling periods separately
datapred<-aggregate(cbind(fit,lwr,upr)~SLA+Treatment,FUN=mean,data= datapred)
datavpred<-slavpred(m2,comp) # takes two-three minutes to run
#average across seasons for more clarity, remove to look at the two sampling periods separately
datavpred<-aggregate(cbind(fit,lwr,upr)~SLAv+Treatment,FUN=mean,data= datavpred)
...
diagpred<-ggplot(aes(x=SLA, y=fit, colour=Treatment), data=datapred)+
  ...
  ylim(c(-9,-5))+ #might need to change it if scaled
  xlab(" ")+
  theme_bw()
...
datapred<-slapred(m3,comp)
#average across seasons for more clarity, remove to look at the two sampling periods separately
datapred<-aggregate(cbind(fit,lwr,upr)~SLA+Treatment,FUN=mean,data= datapred)
datavpred<-slavpred(m3,comp) #takes two-three minutes each to run
#average across seasons for more clarity, remove to look at the two sampling periods separately
datavpred<-aggregate(cbind(fit,lwr,upr)~SLAv+Treatment,FUN=mean,data= datavpred)
...
asympred<-ggplot(aes(x=SLA, y=fit, colour=Treatment), data=datapred)+
  ...
  geom_line(size=0.8)+
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.1)+
  ylim(c(1.3,2.0))+ #might need to change it if scaled  ylab("Asymmetry")+
  xlab(" ")+
  theme_bw()
...
datapred<-slapred(m4,comp)
#average across seasons for more clarity, remove to look at the two sampling periods separately
datapred<-aggregate(cbind(fit,lwr,upr)~SLA+Treatment,FUN=mean,data= datapred)
datavpred<-slavpred(m4,comp) #takes two-three minutes each to run
#average across seasons for more clarity, remove to look at the two sampling periods separately
datavpred<-aggregate(cbind(fit,lwr,upr)~SLAv+Treatment,FUN=mean,data= datavpred)
...
datapred<-slapred(m5,comp)
#average across seasons for more clarity, remove to look at the two sampling periods separately
datapred<-aggregate(cbind(fit,lwr,upr)~SLA+Treatment,FUN=mean,data= datapred)
datavpred<-slavpred(m5,comp) #takes two-three minutes each to run
#average across seasons for more clarity, remove to look at the two sampling periods separately
datavpred<-aggregate(cbind(fit,lwr,upr)~SLAv+Treatment,FUN=mean,data= datavpred)
...
datapred<-slapred(m1,comp)
#average across seasons for more clarity, remove to look at the two sampling periods separately
datapred<-aggregate(cbind(fit,lwr,upr)~SLA+Treatment,FUN=mean,data= datapred)
datavpred<-slavpred(m1,comp) #takes two-three minutes each to run
#average across seasons for more clarity, remove to look at the two sampling periods separately
datavpred<-aggregate(cbind(fit,lwr,upr)~SLAv+Treatment,FUN=mean,data= datavpred)
...
```{r, plot of fig. 3, fig.width = 14, fig.height = 8, fig.align ='center' }
...
## a. Plotting networks and modules {-}

In order to visualize the impact of fungicide on modularity of slow and fast growing species and plot Supplementary figure 1, we represented networks as igraph objects and showed how the `cluster_optimize()` function creates groups of species that interact strongly together, maximizing the modularity. We also added the M, modularity value for each network.

- Slow growing species modules, fungicide effect
```{r, slow growing network modules computing}

#Slow growing species
netlist<-networkplot(as.data.frame(c(slowsp)), length(slowsp)) #personal function defined in "function_competition_networks.R"
#it returns a list of igraph objects and a list of species cluster membership
#adding treatments as names of the list
names(netlist)<-rep(names(all),each = 2)
#slow growing species color code
colslow<-rep("turquoise4", lenght.out=length(slowsp))

#Control network plot
gcont<-netlist$`June C`
#saves species cluster membership
clucont<-netlist[[2]]
#formats in a dataframe
indclasscont<-data.frame(values=c(slowsp),ind=as.character(clucont$membership))
#defines color of clusters
colcont<-c("1"="antiquewhite2", "2"="seashell3", "3"="peachpuff3")
#matches the colors with the membership
V(gcont)$color <- colcont[indclasscont$ind[match(V(gcont)$name,indclasscont$values)]]
#color code of interactions
E(gcont)$color<-"mediumorchid4"
E(gcont)[weight>=0]$color<-"sienna1"

#reorders species for better graphical representation
gcont1 <- igraph::permute(gcont,c(V(gcont)[2], 
                                  V(gcont)[1],
                                  V(gcont)[3],  
                                  V(gcont)[7], 
                                  V(gcont)[9],  
...
#saves modularity
modu1<-modularity(gcont,membership(clucont), weights = abs(E(gcont)$weight))

#Fungicide network plot
gfun<-netlist$`June F`
#saves species cluster membership
clufun<-netlist[[6]]
#formats in a dataframe
indclassfun<-data.frame(values=c(slowsp),ind=as.character(clufun$membership))
#defines color of clusters
colfun<-c("1"="seashell3", "2"="antiquewhite2", "3"="peachpuff3")
#matches the colors with the membership
V(gfun)$color <- colfun[indclassfun$ind[match(V(gfun)$name,indclassfun$values)]]
#color code of interactions
E(gfun)$color<-"mediumorchid4"
E(gfun)[weight>=0]$color<-"sienna1"

#reorders species for better graphical representation
gfun1 <- igraph::permute(gfun,c(V(gfun)[1], 
                                  V(gfun)[9],
                                  V(gfun)[3],  
                                  V(gfun)[4], 
                                  V(gfun)[5],  
...
#saves modularity
modu2<-modularity(gfun,membership(clufun), weights = abs(E(gfun)$weight))

```

```{r, supplementary fig 1 slow growing network plot, fig.width = 8, fig.height=12, echo=FALSE}

par(mfrow=c(2,1))

plot.igraph(gcont1,edge.width=abs(E(gcont)$weight),edge.arrow.size=0.1,edge.arrow.width=1.5,edge.curved=0.2,
            edge.color=E(gcont1)$color, vertex.size=20,vertex.label.cex=1.5,vertex.label.color=colslow,
            mark.groups=clucont,mark.col=colcont,layout=layout.circle, mark.border = "white")
title(main="Slow species, control")
text(-1,-1, paste("M=", round(modu1, digits=3), sep = ""), cex = 1.5)


plot.igraph(gfun1,edge.width=abs(E(gfun)$weight),edge.arrow.size=0.1,edge.arrow.width=1.5,edge.curved=0.2,
            edge.color=E(gfun1)$color, vertex.size=20,vertex.label.cex=1.5,vertex.label.color=colslow,
            mark.groups=clufun,mark.col=colfun,layout=layout.circle, mark.border = "white")
title(main="Slow species, fungicide")
text(-1,-1, paste("M=", round(modu2, digits=3), sep = ""), cex = 1.5)

```

- Fast growing species modules, fungicide effect
```{r, fast growing network modules computing}

#Slow growing species
netlist<-networkplot(as.data.frame(c(fastsp)), length(fastsp))#personal function defined in "function_competition_networks.R"
#it returns a list of igraph objects and a list of species cluster membership
#adding treatments as names of the list
names(netlist)<-rep(names(all),each = 2)
#fast growing species color code
colfast<-rep("chartreuse4", lenght.out=length(fastsp))
...
clucont<-netlist[[2]]
#formats in a dataframe
indclasscont<-data.frame(values=c(fastsp),ind=as.character(clucont$membership))
#defines color of clusters
colcont<-c("1"="honeydew3", "2"="khaki3", "3"="navajowhite3")
#matches the colors with the membership
V(gcont)$color <- colcont[indclasscont$ind[match(V(gcont)$name,indclasscont$values)]]
#color code of interactions
E(gcont)$color<-"mediumorchid4"
...
#no need for permutations here

#saves modularity
modu3<-modularity(gcont,membership(clucont), weights = abs(E(gcont)$weight))
...
clufun<-netlist[[6]]
#formats in a dataframe
indclassfun<-data.frame(values=c(fastsp),ind=as.character(clufun$membership))
#defines color of clusters
colfun<-c("1"="honeydew3", "2"="navajowhite3", "3"="khaki3")
#matches the colors with the membership
V(gfun)$color <- colfun[indclassfun$ind[match(V(gfun)$name,indclassfun$values)]]
#color code of interactions
E(gfun)$color<-"mediumorchid4"
...
#saves modularity
modu4<-modularity(gfun,membership(clufun), weights = abs(E(gfun)$weight))

```

```{r, supplementary fig 1 fast growing network plot, fig.width = 8, fig.height=12, echo=FALSE}

par(mfrow=c(2,1))

plot.igraph(gcont,edge.width=abs(E(gcont)$weight),edge.arrow.size=0.1,edge.arrow.width=1.5,edge.curved=0.2,
            edge.color=E(gcont)$color, vertex.size=20,vertex.label.cex=1.5,vertex.label.color=colfast,
            mark.groups=clucont,mark.col=colcont,layout=layout.circle, mark.border = "white")
title(main="Fast species, control")
text(-1,-1, paste("M=", round(modu3, digits=3), sep = ""), cex = 1.5)


plot.igraph(gfun1,edge.width=abs(E(gfun)$weight),edge.arrow.size=0.1,edge.arrow.width=1.5,edge.curved=0.2,
            edge.color=E(gfun1)$color, vertex.size=20,vertex.label.cex=1.5,vertex.label.color=colfast,
            mark.groups=clufun,mark.col=colfun,layout=layout.circle, mark.border = "white")
title(main="Fast species, fungicide")
text(-1,-1, paste("M=", round(modu4, digits=3), sep = ""), cex = 1.5)

```

## b. Network metrics for communities of 5, 7, 11 and 15 species {-}
...
```
<br>
From the results of these multimembership model, we can plot the Supplementary figure 4, which present the impact of treatment and SLA on mixed communities of different size across June and August 2020.
<br>
```{r, plot of supp fig. 4, fig.width = 14, fig.height = 16, fig.align ='center', echo=FALSE}
...
```
<br>
And the Supplementary figure 5, which present the impact of treatment and SLA variance on mixed communities of different size across June and August 2020.
<br>
```{r, plot of supp fig. 5, fig.width = 14, fig.height = 16, fig.align ='center', echo=FALSE}
...
First, we used PaNDiv experiment cover data from 2020, to fit a linear model between the species average competitive response and the change in their cover with fungicide addition in June 2020.
...
```
We can then plot Supplementary figure 6,
```{r,plot supplementary figure 6, fig.align ='center', fig.height=10, fig.width=10, echo = FALSE}
...
effect<-ggplot(data=bmatrixall,aes(x=bmatrix,y=bmatrixn))+
  ...
  labs(title = "Competitive effect with treatment June 2020", 
       y="Competitive change with treatment : Nitrogen (green), Fungicide (blue), Combined (purple)", x="Competitive effect in control conditions")+
...
response<-ggplot(data=amatrixall,aes(x=amatrix,y=amatrixn))+
  geom_smooth(method="lm",color="#7CAE00", fill="lightgray", alpha=0.3, linetype="dashed")+
  geom_smooth(aes(x=amatrix,y=amatrixf),method="lm",color="#00BFC4", fill="lightgray", alpha=0.3, linetype="dashed")+
  geom_smooth(aes(x=amatrix,y=amatrixnf),method="lm",color="#C77CFF", fill="lightgray", alpha=0.3, linetype="dashed")+
  geom_point(color="#7CAE00")+
  geom_text(aes(label=matsp),color="#7CAE00", hjust=1.5)+
  ...
  geom_abline(intercept = 0, slope=1)+
  xlim(c(-90,10))+ylim(c(-90,10))+
  labs(title = "Competitive response with treatment June 2020", 
       y=" ", x="Competitive response in control conditions")+
  scale_color_manual(name="Treatments",values = c("#7CAE00","#00BFC4"), breaks=c("Nitrogen","Fungicide"))+
  theme_clean()+
  theme(plot.background=element_blank())
```

```{r, plot supplementary figure 7, fig.align ='center', fig.width=13, fig.height=8, echo = FALSE}

ggarrange(effect, response)

```

- insight on the SLA shift in PaNDiv after nitrogen addition


```{r}

tg <- read_excel("data/SLA shift.xlsx")
tg$Delta_SLA<-as.numeric(tg$Delta_SLA)
tg <- na.omit(tg)

tg$Treatment<-NA
for (i in 1:nrow(tg)) {
  if (tg$Nitrogen[i] == 0 & tg$Fungicide[i] == 0) {
    tg$Treatment[i]<- "C"
  }
  if (tg$Nitrogen[i] == 0 & tg$Fungicide[i] == 1)  {
    tg$Treatment[i]<- "F"
  }
  if (tg$Nitrogen[i] == 1 & tg$Fungicide[i] == 0)  {
    tg$Treatment[i]<- "N"
  }
   if (tg$Nitrogen[i] == 1 & tg$Fungicide[i] == 1)  {
    tg$Treatment[i]<- "NF"
  }
  
}

tg$Treatment<-as.factor(tg$Treatment)
tg$Treatment<-factor(tg$Treatment, levels = c("C","N","F","NF"))

tgc <- Rmisc::summarySE(tg, measurevar="Delta_SLA", groupvars=c("Treatment","Year")) ##sd,seֵ
pd <- position_dodge(0.3)
a1 <- ggplot(tgc, aes(x=Year, y=Delta_SLA, colour=Treatment, group=Treatment)) + 
...
```{r, plot supplementary figure 8, fig.align ='center', fig.width=13, fig.height=8, echo = FALSE}
