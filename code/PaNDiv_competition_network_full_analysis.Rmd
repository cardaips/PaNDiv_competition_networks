...
#average across seasons for more clarity
...
With the model predictions we can plot Fig.2,
...
First, we used PaNDiv experiment cover data from 2020, to fit a linear model between the species average competitive response and the change in their cover in June 2020.
...
plot(x$diff~x$bmatrix, xlab= "Competitive response", ylab = "Change in %cover with Fungicide")
...
bmatrixall[6,]<-NA #check if holcus is driving the pattern
...
## h. Plotting networks and modules {-}

- Slow growing species modules
```{r, slow growing network modules computing}

#Slow growing species
netlist<-networkplot(as.data.frame(c(slowsp)), length(slowsp))
...
```{r, slow growing network plot, fig.width = 8, fig.height=12, echo=FALSE}
...
- Fast growing species modules
```{r, fast growing network modules computing}

#Slow growing species
netlist<-networkplot(as.data.frame(c(fastsp)), length(fastsp))
names(netlist)<-rep(names(all),each = 2)
...
```{r, fast growing network plot, fig.width = 8, fig.height=12, echo=FALSE}
...
```{r, plot of supp fig. 4 and 5, fig.width = 14, fig.height = 16, fig.align ='center'}
