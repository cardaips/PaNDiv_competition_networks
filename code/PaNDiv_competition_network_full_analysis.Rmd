...
```{r, format the dataframe and run richness multi membership models}
#format the table
...
allrich_data$o.richness<-as.factor(as.numeric(allrich_data$Richness))
levels(allrich_data$o.richness)<-c(5,7,11,15)
allrich_data$o.richness<-ordered(allrich_data$o.richness)
...
#Modularity
lmod <- lFormula(Modularity~Nitrogen*Fungicide*o.richness+Nitrogen*Fungicide*(SLA+SLAv)+(1|Species), data=allrich_data)
lmod$reTrms$Zt <- lmod$reTrms$Ztlist[[1]] <- Matrix(t(speciesmat))
devfun <- do.call(mkLmerDevfun, lmod)
opt <- optimizeLmer(devfun)
m1 <- mkMerMod(environment(devfun), opt, lmod$reTrms, fr = lmod$fr)
