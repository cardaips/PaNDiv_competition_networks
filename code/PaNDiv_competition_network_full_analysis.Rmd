...
diagpred<-ggplot(aes(x=SLA, y=fit, colour=Treatment), data=datapred)+
  ...
  ylim(c(-4,-8))+ #might need to change it if scaled
  xlab(" ")+
  theme_bw()
...
skewpred<-ggplot(aes(x=SLA, y=fit, colour=Treatment), data=datapred)+
  ...
  geom_line(size=0.8)+
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.1)+
  ylim(c(-1.5,1.5))+ #might need to change it if scaled
  ylab("Skewness")+
  xlab(" ")+
  theme_bw()
...
ginipred<-ggplot(aes(x=SLA, y=fit, colour=Treatment), data=datapred)+
  #facet_grid(~Season)+ #add to look at the two sampling periods separately
  ylim(c(0.35,0.65))+ #might need to change it if scaled
  geom_line(size=0.8)+
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.1)+
  ylab("Evenness")+
  xlab(" ")+
...
modupred<-ggplot(aes(x=SLA, y=fit, colour=Treatment), data=datapred)+
  ...
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.1)+
  ylab("Modularity")+
  ylim(c(0,0.2))+ #might need to change it if scaled
  theme_bw()
...
This code is also long to run, **around 5 hours**, the output can be found directly below in the "multimembership_results_richness_5-7-11-15_species.Rdata".
...
- insight on the competitive effect versus response at the species level
