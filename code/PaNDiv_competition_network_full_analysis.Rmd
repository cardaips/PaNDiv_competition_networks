...
```{r,plot supplementary figure 5, fig.align ='center', fig.height=10, fig.width=10, echo = FALSE}
...
Secondly, we wanted to test whether treatment impacted competition coefficients at the species level. For this, we plotted the change in competitive effect/response with treatment fo every species. The black line represents the values that would be obtained if there was no impact of the treatments.The dotted colored lines represent the obtained values by treatment (difference was not significant).
...
effect<-ggplot(data=bmatrixall,aes(x=bmatrix,y=bmatrixn))+
  ...
  labs(title = "Changes in competitive effect with treatment June 2020", 
...
response<-ggplot(data=amatrixall,aes(x=amatrix,y=amatrixn))+
  ...
  geom_abline(intercept = 0, slope=1)+
  xlim(c(-90,10))+ylim(c(-90,10))+
  labs(title = "Changes in competitive response with treatment June 2020", 
...
```{r, plot supplementary figure 6, fig.align ='center', fig.width=13, fig.height=8, echo = FALSE}
