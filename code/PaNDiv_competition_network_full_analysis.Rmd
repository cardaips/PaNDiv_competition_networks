...
n<-list(5,7,11,15,18) #assign numbers between 1 and 18 for sp richness of sub matrix
allrich<-NA
...
datapred<-slapred(m1,allrich_data)
...
modupred<-ggplot(aes(x=SLA, y=fit, colour=Treatment), data=datapred)+
  ...
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.1)+
  ylab("Modularity")+
  #scale_colour_manual(breaks= c("Control","Nitrogen","Fungicide"),values = c("#F8766D","#7CAE00","#00BFC4"))+
  theme_bw()
modupred
```
